...
app.get('/api/orders', async (req, res) => {
  try {
    ...
    for (const o of orders.rows) {
      const items = await pool.query('SELECT * FROM order_items WHERE order_id = $1 ORDER BY id', [o.id]);
...
app.post('/api/orders', async (req, res) => {
  ...
  try {
    ...
    if (!items || !Array.isArray(items) || !items.length) return res.status(400).json({ ok: false, error: 'items bosh bolmasin' });
    ...
    for (const it of items) {
      ...
      if (productId) {
        ...
        if (p.rows.length) { sku = sku || p.rows[0].sku; if (unitPrice <= 0) unitPrice = num(p.rows[0].price); }
      }
      if (!sku) continue;
    ...
    const paid = Math.max(0, num(paid_amount)), debt = Math.max(0, total - paid);
    let status = 'pending';
    if (debt === 0) status = 'paid'; else if (paid > 0) status = 'partial';
    ...
    for (const line of lines) {
      await client.query(`INSERT INTO order_items (order_id, product_id, sku, qty, unit_price, line_total) VALUES ($1,$2,$3,$4,$5,$6)`, [ord.rows[0].id, line.product_id, line.sku, line.qty, line.unit_price, line.line_total]);
    }
    if (paid > 0) {
      await client.query(`INSERT INTO payments (order_id, customer_id, amount, method, note) VALUES ($1,$2,$3,'cash','Boshlangich tolov')`, [ord.rows[0].id, custId, paid]);
    }
...
app.patch('/api/orders/:id/status', async (req, res) => {
  try {
    ...
    const r = await pool.query(`UPDATE orders SET status = $1, admin_note = COALESCE($2, admin_note) WHERE id = $3 RETURNING *`, [status, admin_note !== undefined ? admin_note : null, req.params.id]);
    if (!r.rows.length) return res.status(404).json({ ok: false, error: 'Buyurtma topilmadi' });
...
app.post('/api/payments', async (req, res) => {
  ...
  try {
    ...
    if (order_id) {
      ...
      const debts = await client.query(`SELECT * FROM orders WHERE customer_id = $1 AND debt_amount > 0 ORDER BY ordered_at ASC`, [custId]);
      for (const o of debts.rows) {
        if (remaining <= 0) break;
        const pay = Math.min(num(o.debt_amount), remaining);
        const newDebt = num(o.debt_amount) - pay, newPaid = num(o.paid_amount) + pay;
        const st = newDebt === 0 ? 'paid' : 'partial';
        await client.query(`UPDATE orders SET paid_amount = $1, debt_amount = $2, status = $3 WHERE id = $4`, [newPaid, newDebt, st, o.id]);
        remaining -= pay;
      }
    }
