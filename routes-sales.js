module.exports = function(app, pool, helpers) {
  ...
  app.put('/api/orders/:id/status', async (req, res) => {
    try {
      ...
      if (!next || !ALLOWED_STATUSES.includes(next)) {
        return res.status(400).json({ ok: false, error: 'status: ' + ALLOWED_STATUSES.join(', ') });
      }
      ...
      if ((prev === 'cancelled' || prev === 'rejected') && SOLD_STATUSES.includes(next)) {
        return res.status(400).json({ ok: false, error: 'Bekor/rad etilgan buyurtmani qayta tasdiqlab bolmaydi' });
      }
  ...
  app.get('/api/payments', async (req, res) => {
    try {
      ...
      const r = await pool.query('SELECT * FROM payments ORDER BY id DESC LIMIT $1', [limit]);
  ...
  app.post('/api/payments', async (req, res) => {
    const client = await pool.connect();
    try {
      const { order_id, customer_id, amount, method, note } = req.body;
      const sum = num(amount);
      if (sum <= 0) return res.status(400).json({ ok: false, error: 'amount musbat bolsin' });
      if (!order_id && !customer_id) return res.status(400).json({ ok: false, error: 'order_id yoki customer_id kerak' });
      await client.query('BEGIN');
  ...
  app.post('/api/expenses', async (req, res) => {
    try {
      ...
      if (!category) return res.status(400).json({ ok: false, error: 'category majburiy' });
