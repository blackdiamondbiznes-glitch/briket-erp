...
app.delete('/api/batches/:id', async (req, res) => {
  try {
    ...
    if (num(check.rows[0].packed_kg) > 1) return res.status(400).json({ ok: false, error: 'Qadoqlangan partiya ochirilmaydi' });
...
app.post('/api/packaging', async (req, res) => {
  ...
  try {
    ...
    if (batchId) {
      ...
      const actives = await client.query(`SELECT * FROM batches WHERE status = 'active' AND remaining_kg > 0 ORDER BY created_at ASC`);
      let need = totalKg;
